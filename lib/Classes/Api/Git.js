...
exports.Git = void 0;
exports.sanitizeIdentitySuffix = sanitizeIdentitySuffix;
/*
 ...
 */
function sanitizeIdentitySuffix(value) {
    return value
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');
...
class Git {
    ...
    isInstalled() {
        ...
        const executableMarker = this._context.platform.choose({ windows: 'git.exe', linux: '/git' });
     ...
     */
    getWorktreeInfo() {
        const none = { isWorktree: false, name: null, topLevel: null, mainWorkTreePath: null };
        if (!this.isInstalled()) {
            return none;
        }
        const cd = this._context.platform.choose({ windows: 'cd /d ', linux: 'cd ' });
        const dir = this._context.rootDirectory;
        const run = (args) => childProcess.execSync(cd + '"' + dir + '" && git ' + args, { 'stdio': 'pipe' })
            .toString('utf8').trim();
        try {
     ...
     */
    getRemoteBranches(target, skipPullRequests) {
     ...
     * @param directory
     */
    stageFilesInDirectory(directory) {
        const command = this._context.platform.choose({ windows: 'cd /d ', linux: 'cd ' }) +
            '"' + directory + '" && git add .';
        childProcess.execSync(command);
        return this;
