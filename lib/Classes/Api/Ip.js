...
class Ip {
    ...
    static ip2long(argIP) {
        ...
        let argIPParts = argIP.match(pattern); // Verify argIP format.
        if (!argIPParts) {
            // Invalid format.
            return false;
        }
        ...
        return argIPParts[1] * (argIPParts[0] === 1 || 16777216) +
            argIPParts[2] * (argIPParts[0] <= 2 || 65536) +
            argIPParts[3] * (argIPParts[0] <= 3 || 256) +
            argIPParts[4] * 1;
    }
    /**
