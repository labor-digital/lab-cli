...
class Config {
     ...
     */
    get(key, fallback) {
        return (0, radashi_1.get)(this.config, Array.isArray(key) ? key.join('.') : key, fallback);
    }
    /**
     ...
     */
    extend(mergeConfig) {
        if (!(0, radashi_1.isObject)(mergeConfig)) {
            throw new Error('The given config is not a plain object!');
        }
        this.config = (0, radashi_1.assign)(this.config, mergeConfig);
        return this;
    }
}
