...
export class NpmCommand {
	...
	protected getPackageJsonPath(context: AppContext): Promise<string> {
		return new Promise<string>((resolve, reject) => {
			...
			if (isString(packageJsonPath)) return resolve(packageJsonPath);
	 ...
	 * @param otherArgs
	 */
	protected executeRunStatement(packageJsonPath: string, context: AppContext, run: string, otherArgs: Array<string>): Promise<AppContext> {
		// Get the contents
	...
	protected askForPackageJsonPath(context: AppContext): Promise<string> {
		return inquirer.prompt([
			...
			},
			{
				...
				validate: input => {
					if (fs.existsSync(input) && input.match(/package\.json$/)) return true;
