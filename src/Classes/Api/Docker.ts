...
export class Docker {
			...
			this._context.platform.choose({
				windows: () => {
					...
					for (let i = 0; i < options.length; i++) {
						if (!fs.existsSync(options[i])) continue;
						executablePath = options[i];
						break;
					}
					if (isNull(executablePath)) throw new Error("Could not find a matching docker executable!");
	...
	public stopEngine(force?: boolean): Promise<void> {
		return this.timeoutHandler(() => {
			return new Promise((resolve, reject) => {
				this._context.platform.choose({
					windows: () => {
						if (force !== true && !this.isRunning) return resolve();
	...
	public attachToContainerShell(containerName: string, shell: string): Promise<void> {
		return new Promise((resolve, reject) => {
			console.log(chalk.yellowBright("Attaching to shell of: " + containerName + " - To close the shell type \"exit\" and hit enter."));
			try {
				const command = "docker exec -ti " + containerName + " " + shell;
				childProcess.execSync(command, {stdio: "inherit"});
				resolve();
			} catch (e) {
				if (e.status !== 1) return resolve();
				return reject(e);
			}
		});
	}
