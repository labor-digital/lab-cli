...
export class DockerApp {
	...
	public get dockerComposeFile(): string | undefined {
		if (this._dockerComposeFile === "-1") return undefined;
		...
		if (!isUndefined(this._dockerComposeFile)) return this._dockerComposeFile;
	...
	public get dockerComposeOverrideFile(): string | undefined {
		if (this._dockerComposeOverrideFile === "-1") return undefined;
		forEach(DockerApp.dockerComposeOverrideFiles, (filename: string) => {
			filename = path.join(this._context.rootDirectory, filename);
			if (!fs.existsSync(filename)) return;
			this._dockerComposeOverrideFile = filename;
		});
		if (!isUndefined(this._dockerComposeOverrideFile)) return this._dockerComposeOverrideFile;
	...
	public get containerName(): string {
		...
		if (!isUndefined(serviceKey)) {
			...
			forEach(services, (service: PlainObject) => {
				if (service.key !== serviceKey) return;
				containerName = service.containerName;
				return false;
			});
			if (isString(containerName)) return containerName;
		}
	...
	public get serviceKey(): string {
		...
		if (!isUndefined(serviceKey)) return serviceKey;
		
		// Find the service by the container name
		const containerName = this.containerName;
		const services = this.dockerCompose.getServiceList();
		forEach(services, (service: PlainObject) => {
			if (service.containerName !== containerName) return;
			serviceKey = service.key;
			return false;
		});
		if (!isString(serviceKey)) return "app";
		return serviceKey;
	}
	...
	public initialize(): Promise<DockerApp> {
		...
		return this.startDockerIfRequired().then(() => {
			...
			if (isEmpty(config)) return this.runInit();
	...
	public calculateDockerFileHash(): string {
		...
		forEach([...DockerApp.dockerComposeRootFiles, ...DockerApp.dockerConfigFiles], (file: string) => {
			...
			if (!fs.existsSync(filename)) rawHash.push("0");
			else rawHash.push(md5(fs.readFileSync(filename).toString("utf-8")));
		});
	...
	protected startDockerIfRequired(): Promise<void> {
		if (this._api.isRunning) return Promise.resolve();
		...
		return inquirer.prompt({
			...
			if (!answers.startDocker) return Promise.reject(new Error("Sorry, this can't be done without docker running!"));
