...
export class ProjectNameInputWizard {
		...
		return new Promise<string>((resolve, reject) => {
			inquirer.prompt([
				...
				},
				{
					...
					validate: input => {
						if (input.length === 0) return "The input can't be empty!";
						if (input.replace(/[^0-9_\-a-z.]/g, "") !== input) return "The name can only be alpha numerical with dashes, periods and underscores";
						return true;
					}
				},
				{
					...
					validate: input => {
						if (input.length === 0) return "The input can't be empty!";
						if (input.replace(/[^-]/g, "").length !== 2) return "The name should consist of three parts, separated with a dash!";
						if (input.replace(/[^0-9_\-a-z]/g, "") !== input) return "The name can only be alpha numerical with dashes and underscores";
						return true;
					}
				},
				{
				...
				if (answers.projectNameInputType === "wizard")
					...
					projectName = "playground-" + (Math.floor(Math.random() * 99999) + (new Date()).getTime()) + "-" + (Math.floor(Math.random() * 99999));
				else {
					...
					while (parts.length < 3) {
						...
						if (part === "") continue;
						if (typeof part === "undefined") return reject("Could not read project name by directory, because the path is to short!");
				...
				return inquirer.prompt([
					...
					if (!answers.ok) ProjectNameInputWizard.run(initialQuestion, context).then(resolve);
					else return resolve(projectName);
				});
			});
		});
	}
	...
	protected static wizardInputValidate(input: string): string | boolean {
		if (input.length === 0) return "The input can't be empty!";
		if (input.replace(/[^0-9_\-a-z]/g, "") !== input) return "The name can only be alpha numerical with underscores";
		return true;
	}
}
