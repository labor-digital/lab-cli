...
export class DockerComposeServiceSelectWizard {
	 ...
	 */
	public static run(dockerCompose: DockerCompose, forWhat: string, suggestedServiceKey?: string, currentContainerName?: string): Promise<string> {
		...
		if (isString(currentContainerName)) {
			...
			if (isString(newSuggestedServiceKey)) suggestedServiceKey = newSuggestedServiceKey;
		}
		...
		return inquirer.prompt([
			...
			if (answers.ok) return Promise.resolve(
	 ...
	 * @param containerName
	 */
	protected static convertCurrentContainerNameToServiceKey(dockerCompose: DockerCompose, containerName: string): string | undefined {
		const services = dockerCompose.getServiceList();
	...
	protected static convertServiceKeyToContainerName(dockerCompose: DockerCompose, serviceKey: string): string {
		...
		if (!isString(containerName)) throw new Error("Failed to convert serviceKey: " + serviceKey + " to a container name!");
