	...
	public handle(context: AppContext): Promise<AppContext> {
		return new Promise<AppContext>((resolve, reject) => {
			...
			if (!this._commandWasExecuted) {
				console.error(chalk.redBright("Invalid command \"" + parseResult.args[0] + "\" given! \r\nUse \"labor help\" to see a list of supported commands!"));
				resolve(context);
			}
		});
	}
		...
		context.emitSequentialHook(AppEventList.BEFORE_ACTION, {
			...
			return new Promise((resolve1, reject1) => {
				...
				try {
					...
					if (isFunction(handler.default)) handler = handler.default;
					else if (getListKeys(handler).length === 1) handler = handler[getListKeys(handler)[0]];
					else return reject1(new Error("Invalid command handler class for command: " + command.signature));
					...
					if (!(result instanceof Promise))
						return reject1(new Error("The command \"" + command.signature + "\" did not return a promise in its action method: \"" + command.options.action + "\""));
